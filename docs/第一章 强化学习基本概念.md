...
3. **Reward**：奖励可以用来衡量不同状态的价值。我们想要达到的状态，奖励可以设置大一点；不想要的状态，奖励可以设置为0甚至设置为负数;
...
* 我们可以看到 $v_{\pi_3}(s_1)$ 就是 $s_1$ 状态下，各轨迹累积回归的期望值.

## 五、Episode

一个**Episode（片段）**是指从初始状态开始，直到到达终止状态的完整交互序列。在有终止状态的任务（Episodic Task）中，每次交互以Episode为单位；而在持续性任务（Continuing Task）中，交互过程无限延续，此时必须使用折扣化回报。

## 六、价值函数

### （1）状态值函数

$v(s)$ 是状态 $s$ 的期望回报(Expected Return)：
